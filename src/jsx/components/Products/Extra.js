...
const Extra = (props) => {
  ...
  const save = (e) => {
    axios.post("/api/InsertExtra", insert).then((res) => {
  ...
  const fetch = (e, id) => {
    e.preventDefault();
    axios.get(`/api/EditExtra/${id}`).then((res) => {
      if (res.data.status === 200) {
  ...
  const update = (e) => {
    e.preventDefault();
    axios.post("/api/UpdateExtra", edit).then((res) => {
      if (res.data.status === 200) {
        setEdit([]);
        setInsert([]);
  ...
  const deleteIngredient = (e, id) => {
    ...
    swal({
      ...
      if (willDelete) {
        axios.delete(`/api/DeleteExtra/${id}`).then((res) => {
          if (res.data.status === 200) {
            swal("Success", res.data.message, "success");
            setInsert([]);
          } else if (res.data.status === 404) {
  ...
  return (
    <Fragment>
      <CBreadcrumb style={{ "--cui-breadcrumb-divider": "'>'" }}>
        <CBreadcrumbItem active>{t("extras")}</CBreadcrumbItem>
      </CBreadcrumb>
      {/* insert */}
      <Modal className="fade" show={modalCentered}>
        ...
        >
          <Modal.Header>
            <Modal.Title>{t("add_extra")}</Modal.Title>
            <Button
      ...
      <Modal className="fade" show={editmodalCentered}>
        <Form onSubmit={update} method="POST">
          <Modal.Header>
            <Modal.Title>{t("edit_extra")} </Modal.Title>
            <Button
        ...
        <div className="col-xl-12 col-xxl-12 col-lg-12 col-sm-12">
          <div className="card">
            <div className="card-header border-0">
              <div>
                <h4 className="card-title mb-2">{t("extras")}</h4>
              </div>
